...
def plot_one_box(box, im, color=(128, 128, 128), txt_color=(255, 255, 255), label=None, line_width=3, use_pil=False):
    ...
    if use_pil or not is_ascii(label):  # use PIL
        ...
        if label:
            font_path = os.path.join(str(Path(__file__).parent.resolve()), "arial.ttf")
            font_path = os.path.join(str(Path(__file__).parent.resolve()), "NotoSansJP-Regular.otf")
