...
from mask_rcnn.helpers import extract_warped_masks, visualize_detections
import matplotlib.pyplot as plt
from helpers import objdict
from data import LoadImagesDataset, collate_fn
from model import load_maskrcnn
import cv2

def maskrcnn_predict(model, device: str, data_loader)-> list :
    """
...
def inference(cfg):
    ...
    save_masks, enable_visualize, enable_log, save_boxes =  cfg.weights, cfg.device, cfg.score_thres, \
                                                    cfg.output, cfg.source, cfg.batch_size, cfg.num_workers, \
                                                    cfg.save_masks, cfg.enable_visualize, cfg.enable_log, cfg.save_boxes
    images_with_detections = []
    images_with_no_detections = []


    # set output directories
    if enable_log or save_boxes or save_masks or enable_visualize:
        assert isinstance(output, str), "Output must be provided when extracts_masks or visualize is enabled"
        ...
        if save_masks:
            extracted_warped_masks_dir = os.path.join(output, "extracted_warped_masks")
            if os.path.exists(extracted_warped_masks_dir): shutil.rmtree(extracted_warped_masks_dir)
            os.makedirs(extracted_warped_masks_dir)
        if enable_visualize:
    ...
    # load model
    model = load_maskrcnn(weights, device, score_threshold=score_thres)
    ...
    mrcnn_predictor = maskrcnn_predict(model, device, data_loader)
    ...
    for detections_per_batch in tqdm.tqdm(mrcnn_predictor, desc=f"Batch inferencing: ", unit="Batch", total=len(data_loader)):
        ...
        for detections_per_image in detections_per_batch:
            ...
            masks = detections_per_image["masks"]
            atleast_one_detection = 1 if torch.is_tensor(masks) and masks.shape[0] else 0
            ...
            if atleast_one_detection:
                ...
                if save_masks:
                    ...
                    [ os.makedirs( os.path.join( extracted_warped_masks_dir, class_name(label) ), exist_ok=True )
                      for label in labels ]
                    extracts = extract_warped_masks(image, masks)
                    for extract, label, score in zip(extracts,labels,scores):
                        new_image_name = f"{score}_{image_name}"
                        plt.imsave(os.path.join(extracted_warped_masks_dir, class_name(label) ,new_image_name), extract)
