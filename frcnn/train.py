...
import sys
from model import save_final_model, get_frcnn_model, load_frcnn
...
def main():
    ...
    if cfg.model_checkpoint is None:
        if cfg.pre_trained_weights:
            model = get_frcnn_model(cfg.num_classes, cfg.pre_trained_weights)
        else:
            raise Exception("Error: config file should have either pre_trained_weights or model_checkpoint set")
    else:
        model = load_frcnn(cfg.model_checkpoint, cfg.device)

    # set model params
    if not hasattr(model, "idx_to_class"): model.idx_to_class = dataset_train.idx_to_class
    if not hasattr(model, "class_to_idx"): model.class_to_idx = {class_name: idx for idx, class_name in model.idx_to_class.items()}
    if not hasattr(model, "epochs"):       model.epochs = 0
